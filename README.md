...
This integration also has a companion ESPHome project called [Peak Power Indicator](https://github.com/Epyon01P/Peak-Power-Indicator): an unobtrusive LED indicator that physically shows the forecast color gradient in realtime (green → amber → red). However, you can just as easily map the `sensor.peak_power_forecast_color` entity to any RGB LED bulb for the same result.
